...
export class UserComponent extends BaseComponent implements OnInit {
  ...
  search() {
    this.page = 1;
    this.pageSize = 5;
    this._api.post('/api/users/search',{page: this.page, pageSize: this.pageSize, hoten: this.formsearch.get('hoten').value, taikhoan: this.formsearch.get('taikhoan').value, ngaysinh: this.formsearch.get('ngaysinh').value}).takeUntil(this.unsubscribe).subscribe(res => {
      this.users = res.data;
      this.totalRecords =  res.totalItems;
      this.pageSize = res.pageSize;
      });
